...
func startTrafficControl(agentContext agentContext, stopCh <-chan struct{}) error {
	if features.DefaultFeatureGate.Enabled(features.TrafficControl) {
		tcController := trafficcontrol.NewTrafficControlController(agentContext.ofClient,
			agentContext.ifaceStore,
			agentContext.ovsBridgeClient,
			agentContext.ovsCtlClient,
			agentContext.trafficControlInformer,
			agentContext.localPodInformer,
			agentContext.namespaceInformer,
