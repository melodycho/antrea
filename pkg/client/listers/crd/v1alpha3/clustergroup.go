...
type ClusterGroupLister interface {
	...
	List(selector labels.Selector) (ret []*v1alpha3.ClusterGroup, err error)
	// Get retrieves the ClusterGroup from the index for a given name.
	// Objects returned here must be treated as read-only.
	Get(name string) (*v1alpha3.ClusterGroup, error)
...
// List lists all ClusterGroups in the indexer.
func (s *clusterGroupLister) List(selector labels.Selector) (ret []*v1alpha3.ClusterGroup, err error) {
	err = cache.ListAll(s.indexer, selector, func(m interface{}) {
		ret = append(ret, m.(*v1alpha3.ClusterGroup))
	})
	return ret, err
}

// Get retrieves the ClusterGroup from the index for a given name.
func (s *clusterGroupLister) Get(name string) (*v1alpha3.ClusterGroup, error) {
	obj, exists, err := s.indexer.GetByKey(name)
	if err != nil {
		return nil, err
	}
	if !exists {
		return nil, errors.NewNotFound(v1alpha3.Resource("clustergroup"), name)
	}
	return obj.(*v1alpha3.ClusterGroup), nil
}
