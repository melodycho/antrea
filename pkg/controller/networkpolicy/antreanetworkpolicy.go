...
func (c *NetworkPolicyController) processAntreaNetworkPolicy(np *crdv1alpha1.NetworkPolicy) *antreatypes.NetworkPolicy {
	...
	for _, at := range np.Spec.AppliedTo {
		appliedToGroupNamesSet.Insert(c.CreateAppliedToGroup(
			np.Namespace, at.PodSelector, at.NamespaceSelector, at.ExternalEntitySelector, appliedToGroupType, np.Name))
	}
	...
	for idx, ingressRule := range np.Spec.Ingress {
		...
		for _, at := range ingressRule.AppliedTo {
			atGroup := c.CreateAppliedToGroup(np.Namespace, at.PodSelector, at.NamespaceSelector, at.ExternalEntitySelector, appliedToGroupType, np.Name)
