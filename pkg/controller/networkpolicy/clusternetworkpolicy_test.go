	...
	tests := []struct {
	...
	}{
		{
			...
			expectedPolicy: &antreatypes.NetworkPolicy{
				...
				Rules: []controlplane.NetworkPolicyRule{
					{
						...
						From: controlplane.NetworkPolicyPeer{
							AddressGroups: []string{getNormalizedUID(toGroupSelector("", &selectorB, &selectorC,
				...
				AppliedToGroups: []string{getNormalizedUID(toGroupSelector("", &selectorA, nil,
					nil, nil).NormalizedName)},
			},
				...
				Rules: []controlplane.NetworkPolicyRule{
					{
						Direction: controlplane.DirectionIn,
						From: controlplane.NetworkPolicyPeer{
							AddressGroups: []string{getNormalizedUID(toGroupSelector("", &selectorB, nil,
								nil, nil).NormalizedName)},
						},
						Services: []controlplane.Service{
							{
					...
					},
					{
						Direction: controlplane.DirectionIn,
						From: controlplane.NetworkPolicyPeer{
							AddressGroups: []string{getNormalizedUID(toGroupSelector("", nil, &selectorC,
								nil, nil).NormalizedName)},
						},
						Services: []controlplane.Service{
							{
				...
				Rules: []controlplane.NetworkPolicyRule{
					{
						Direction: controlplane.DirectionIn,
						From: controlplane.NetworkPolicyPeer{
							AddressGroups: []string{getNormalizedUID(toGroupSelector("", &selectorB,
								nil, nil, nil).NormalizedName)},
						},
						Services: []controlplane.Service{
							{
								Protocol: &protocolTCP,
					...
					},
					{
						Direction: controlplane.DirectionIn,
						From: controlplane.NetworkPolicyPeer{
							AddressGroups: []string{getNormalizedUID(toGroupSelector("", nil,
								&selectorC, nil, nil).NormalizedName)},
						},
						Services: []controlplane.Service{
							{
								Protocol: &protocolTCP,
		...
		},
		{
			...
			expectedPolicy: &antreatypes.NetworkPolicy{
				...
				Rules: []controlplane.NetworkPolicyRule{
					{
						Direction:       controlplane.DirectionIn,
						AppliedToGroups: []string{getNormalizedUID(toGroupSelector("", &selectorA, nil, nil, nil).NormalizedName)},
						From: controlplane.NetworkPolicyPeer{
							AddressGroups: []string{getNormalizedUID(toGroupSelector("", &selectorB, nil, nil, nil).NormalizedName)},
						},
						Services: []controlplane.Service{
							{
								Protocol: &protocolTCP,
					...
					{
						Direction:       controlplane.DirectionIn,
						AppliedToGroups: []string{getNormalizedUID(toGroupSelector("", &selectorB, &selectorC, nil, nil).NormalizedName)},
						From: controlplane.NetworkPolicyPeer{
							AddressGroups: []string{getNormalizedUID(toGroupSelector("", nil, &selectorC, nil, nil).NormalizedName)},
						},
						Services: []controlplane.Service{
							{
								Protocol: &protocolTCP,
				...
				AppliedToGroups: []string{
					getNormalizedUID(toGroupSelector("", &selectorA, nil, nil, nil).NormalizedName),
					getNormalizedUID(toGroupSelector("", &selectorB, &selectorC, nil, nil).NormalizedName),
				},
		...
		},
		{
			...
			expectedPolicy: &antreatypes.NetworkPolicy{
				...
				Rules: []controlplane.NetworkPolicyRule{
					{
						Direction:       controlplane.DirectionIn,
						AppliedToGroups: []string{getNormalizedUID(toGroupSelector("nsA", nil, nil, nil, nil).NormalizedName)},
						From: controlplane.NetworkPolicyPeer{
							AddressGroups: []string{getNormalizedUID(toGroupSelector("nsA", nil, nil, nil, nil).NormalizedName)},
						},
						Services: []controlplane.Service{
							{
								Protocol: &protocolTCP,
					...
					{
						Direction:       controlplane.DirectionIn,
						AppliedToGroups: []string{getNormalizedUID(toGroupSelector("nsB", nil, nil, nil, nil).NormalizedName)},
						From: controlplane.NetworkPolicyPeer{
							AddressGroups: []string{getNormalizedUID(toGroupSelector("nsB", nil, nil, nil, nil).NormalizedName)},
						},
						Services: []controlplane.Service{
							{
								Protocol: &protocolTCP,
					...
					{
						Direction:       controlplane.DirectionIn,
						AppliedToGroups: []string{getNormalizedUID(toGroupSelector("", nil, &metav1.LabelSelector{}, nil, nil).NormalizedName)},
						From: controlplane.NetworkPolicyPeer{
							AddressGroups: []string{getNormalizedUID(toGroupSelector("", nil, &selectorA, nil, nil).NormalizedName)},
						},
						Services: []controlplane.Service{
							{
								Protocol: &protocolTCP,
				...
				AppliedToGroups: []string{
					getNormalizedUID(toGroupSelector("nsA", nil, nil, nil, nil).NormalizedName),
					getNormalizedUID(toGroupSelector("nsB", nil, nil, nil, nil).NormalizedName),
					getNormalizedUID(toGroupSelector("", nil, &metav1.LabelSelector{}, nil, nil).NormalizedName),
				},
				AppliedToPerRule:      true,
		...
		},
		{
			...
			expectedPolicy: &antreatypes.NetworkPolicy{
				...
				Rules: []controlplane.NetworkPolicyRule{
					{
						Direction:       controlplane.DirectionIn,
						AppliedToGroups: []string{getNormalizedUID(toGroupSelector("nsA", &selectorA, nil, nil, nil).NormalizedName)},
						From: controlplane.NetworkPolicyPeer{
							AddressGroups: []string{getNormalizedUID(toGroupSelector("nsA", &selectorA, nil, nil, nil).NormalizedName)},
						},
						Services: []controlplane.Service{
							{
								Protocol: &protocolTCP,
				...
				AppliedToGroups: []string{
					getNormalizedUID(toGroupSelector("nsA", &selectorA, nil, nil, nil).NormalizedName),
					getNormalizedUID(toGroupSelector("nsB", nil, nil, nil, nil).NormalizedName),
				},
				AppliedToPerRule:      true,
...
func TestAddCNP(t *testing.T) {
	...
	matchAllPeerEgress.AddressGroups = []string{getNormalizedUID(toGroupSelector("", nil, &selectorAll, nil, nil).NormalizedName)}
	tests := []struct {
		name               string
		inputPolicy        *crdv1alpha1.ClusterNetworkPolicy
				...
				Rules: []controlplane.NetworkPolicyRule{
					{
						Direction: controlplane.DirectionIn,
						From: controlplane.NetworkPolicyPeer{
							AddressGroups: []string{getNormalizedUID(toGroupSelector("", &selectorB, &selectorC, nil, nil).NormalizedName)},
						},
						Services: []controlplane.Service{
							{
								Protocol: &protocolTCP,
...
func TestDeleteCNP(t *testing.T) {
	selectorA := metav1.LabelSelector{MatchLabels: map[string]string{"foo1": "bar1"}}
	cnpObj := getCNP()
	apgID := getNormalizedUID(toGroupSelector("", &selectorA, nil, nil, nil).NormalizedName)
	_, npc := newController()
	npc.addCNP(cnpObj)
