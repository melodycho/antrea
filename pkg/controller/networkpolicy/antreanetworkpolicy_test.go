	...
	tests := []struct {
	...
	}{
		{
			...
			expectedPolicy: &antreatypes.NetworkPolicy{
				...
				AppliedToGroups: []string{getNormalizedUID(toGroupSelector("ns1", &selectorA, nil,
					nil, nil).NormalizedName)},
			},
		...
		},
		{
			...
			expectedPolicy: &antreatypes.NetworkPolicy{
				...
				Rules: []controlplane.NetworkPolicyRule{
					{
						Direction: controlplane.DirectionIn,
						From: controlplane.NetworkPolicyPeer{
							AddressGroups: []string{getNormalizedUID(toGroupSelector("ns2", &selectorB,
				...
				AppliedToGroups: []string{getNormalizedUID(toGroupSelector("ns2", &selectorA, nil,
					nil, nil).NormalizedName)},
			},
			expectedAppliedToGroups: 1,
		...
		},
		{
			...
			expectedPolicy: &antreatypes.NetworkPolicy{
				...
				Rules: []controlplane.NetworkPolicyRule{
					{
						Direction: controlplane.DirectionIn,
						AppliedToGroups: []string{getNormalizedUID(toGroupSelector("ns3", &selectorA,
							nil, nil, nil).NormalizedName)},
						From: controlplane.NetworkPolicyPeer{
							AddressGroups: []string{getNormalizedUID(toGroupSelector("ns3", &selectorB,
								nil, nil, nil).NormalizedName)},
						},
						Services: []controlplane.Service{
							{
					...
					{
						Direction: controlplane.DirectionIn,
						AppliedToGroups: []string{getNormalizedUID(toGroupSelector("ns3", &selectorB,
							nil, nil, nil).NormalizedName)},
						From: controlplane.NetworkPolicyPeer{
							AddressGroups: []string{getNormalizedUID(toGroupSelector("", nil,
								&selectorC, nil, nil).NormalizedName)},
				...
				AppliedToGroups: []string{
					getNormalizedUID(toGroupSelector("ns3", &selectorB, nil, nil, nil).NormalizedName),
					getNormalizedUID(toGroupSelector("ns3", &selectorA, nil, nil, nil).NormalizedName),
				},
		...
		},
		{
			...
			expectedPolicy: &antreatypes.NetworkPolicy{
				...
				AppliedToGroups: []string{getNormalizedUID(toGroupSelector("ns4", &selectorA, nil,
					nil, nil).NormalizedName)},
			},
			expectedAppliedToGroups: 1,
			expectedAddressGroups:   1,
		},
		{
			...
			expectedPolicy: &antreatypes.NetworkPolicy{
				...
				AppliedToGroups: []string{getNormalizedUID(toGroupSelector("ns5", &selectorA, nil,
					nil, nil).NormalizedName)},
			},
			expectedAppliedToGroups: 1,
...
func TestAddANP(t *testing.T) {
	...
	matchAllPeerEgress.AddressGroups = []string{getNormalizedUID(toGroupSelector("", nil, &selectorAll,
		nil, nil).NormalizedName)}
	tests := []struct {
		name               string
		inputPolicy        *crdv1alpha1.NetworkPolicy
		...
		},
		{
			...
			expPolicy: &antreatypes.NetworkPolicy{
				...
				AppliedToGroups: []string{getNormalizedUID(toGroupSelector("nsB", &selectorA, nil,
					nil, nil).NormalizedName)},
			},
			expAppliedToGroups: 1,
			expAddressGroups:   1,
...
func TestDeleteANP(t *testing.T) {
	selectorA := metav1.LabelSelector{MatchLabels: map[string]string{"foo1": "bar1"}}
	anpObj := getANP()
	apgID := getNormalizedUID(toGroupSelector("test-ns", &selectorA, nil, nil, nil).NormalizedName)
	_, npc := newController()
	npc.addANP(anpObj)
