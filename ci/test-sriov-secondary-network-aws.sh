...
DOCKER_IMAGE_PATH="$THIS_DIR/../antrea-ubuntu.tar"
...
upload_and_load_image() {
    ...
    ssh -o StrictHostKeyChecking=no -i "$AWS_EC2_SSH_KEY_NAME" ubuntu@"$node_ip" << EOF
        ...
        sudo docker load -i /home/ubuntu/$(basename "$image_path")
...
EOF
    echo "IP pool YAML file created at $IP_POOL_YAML."
...
clean_up() {
    ...
    #After deleting the instance, need to wait for a period of time until the ENI status changes to "Available" before it can be deleted.
    sleep 100
