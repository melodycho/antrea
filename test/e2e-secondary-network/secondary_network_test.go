...
func (data *testData) assignIP(clientset *kubernetes.Clientset) error {
	...
	for _, testPod := range data.pods {
		...
		if err != nil {
			return fmt.Errorf("error when getting cluster Node %s: %v", testPod.nodeName, err)
		}
		eni, has := node.Labels["eni-id"]
		if !has {
			return fmt.Errorf("the label `eni-id` not found in cluster Node: %s", testPod.nodeName)
		}
		var podIP net.IP
		_, err = e2eTestData.PodWaitFor(defaultTimeout, testPod.podName, namespace, func(pod *corev1.Pod) (bool, error) {
			if pod.Status.Phase != corev1.PodRunning {
		...
		logs.Infof("assigned private ip addresses %s to interface %s", podIP, eni)
		if err != nil {
